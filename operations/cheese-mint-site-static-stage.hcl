job "cheese-mint-site-static-stage" {
  datacenters = ["mb-hel"]
  type = "batch"
  ...
  group "cheese-mint-site-static-stage-group" {
    count = 1

    task "cheese-mint-site-static-stage-task" {
      driver = "docker"

      config {
        image = "${CONTAINER_REGISTRY_ADDR}/memetic-block/cheese-mint-site:${VITE_VERSION_SHA}"
